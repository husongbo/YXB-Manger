<template>
	<view class="page-body" style="overflow-y: hidden;" :style="{height:MapHeight}">
		<view class="maps" :style="{height:MapHeight}" v-if="OpenMap">
		...
		<view class="page-section page-section-gap" :style="{display:maptrue}">
			<!-- :controls="controlss" -->
            <map style="width: 100%;" :scale="13" :style="{height:mapsheight}"   :circles="circle" :latitude="latitude" :longitude="longitude"  :markers="covers"  @controltap="clickcontrol" @markertap="dianclick" >
            </map>	
        </view>
		...
		<view class="mapdistance" v-if="mappos" :style="{height:gotoheight}">
			<view class="mapdis">
		...
		<view class="CloseMap" style="width: 100%;background: white;" v-if="CloseMaps">
			<view class="Closemap-img" style="display: flex;justify-content: center;margin-top: 30%;">
				<image src="../../../static/app-plus/no.png" mode="" style="width: 200upx;height: 200upx;"></image>
			</view>
        ...
        data() {
			return {
				...
				covers: [{
					latitude:'',
					longitude:'',
					iconPath: '../../../static/login/loca.png',
					label:{
						content:"",
						color:"#FCFCFC",
						// fontSize:20,
				...
				controlss:[{
					...
					iconPath:'../../../static/app-plus/map.png',
					clickable:true
				}],
			scale:10,
			coverlist:[],//数据
...
      	dianclick(e){
		...
		SaveLocation(lat,lon,item){
			...
			var jsontext={
				latitude:lat,
				longitude:lon,
				iconPath: '../../../static/app-plus/tus.png',
				id:item.id,
		...
		maprightmethods(){
			...
			this.mappos=false
			this.controlss[0].position.top='600'
		},
		toRad(d){
